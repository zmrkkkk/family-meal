...
const READ_URL=`https://${G}.github.io/${R}/data.json`;
const WRITE_URL=`https://api.github.com/repos/${G}/${R}/contents/data.json`;
...
async function ld(){
  sS('⏳ 加载中...');
  try{const r=await fetch(READ_URL+'?t='+Date.now());if(!r.ok)throw new Error('empty');
    const d=await r.json();M=d.menu||[];C=d.cats||[];P=d.mems||[];O=d.orders||[];
  }catch(e){/* 云端无数据，用空数组 */}
  let mx=0;M.forEach(d=>{if(d.id>mx)mx=d.id;});nextId=mx>=999?mx+1:1000;
  lc();cart=cart.filter(c=>M.some(d=>d.id===c.id));sc();
  rA();sS(M.length>0?'✅ ('+M.length+'菜)':'📡 请先添加菜品');_dirty=false;
}
async function sv(){
  ...
  try{
    let sha='';try{const cur=await gh(WRITE_URL);sha=cur.sha;}catch(e){}
    await gh(WRITE_URL,{method:'PUT',body:JSON.stringify({message:'update',content,sha:sha||undefined})});
    sS('✅ ('+M.length+'菜)');_dirty=false;
  }catch(e){sS('⚠️ 保存失败');console.error(e);}
  _saving=false;
}
async function gh(url,opts={}){
  const r=await fetch(url,{...opts,headers:{...opts.headers,Authorization:'Bearer '+T,Accept:'application/vnd.github+json'}});
  if(!r.ok)throw new Error(r.status);return r.json();
}
function mark(){_dirty=true;saveCart();setTimeout(()=>sv(),500);}
function sS(m){document.getElementById('ss').textContent=m;}
async function syncNow(){await ld();toast('🔄 已刷新');}
...
async function cO(){const n=document.getElementById('cnn').value.trim();O.unshift({id:Date.now(),time:new Date().toLocaleString('zh-CN'),items:[...cart],total:tP(),note:n});cart=[];document.getElementById('cnn').value='';sc();lC();lM();rCB();rCP();rH();mark();toast('🎉 下单成功');}
...
async function clH(){if(!O.length)return;if(!confirm('清空全部订单？'))return;O=[];mark();rH();toast('🗑️ 已清空');}
...
async function sD(){const id=document.getElementById('di').value,n=document.getElementById('dn').value.trim(),cat=document.getElementById('dcat').value,pr=parseInt(document.getElementById('dpr').value),desc=document.getElementById('dde').value.trim();if(!n){toast('⚠️ 请输入菜名');return;}if(!cat){toast('⚠️ 请选择分类');return;}if(isNaN(pr)||pr<0){toast('⚠️ 请输入价格');return;}if(id){const d=M.find(x=>x.id===parseInt(id));d.n=n;d.c=cat;d.p=pr;d.d=desc||n;d.img=tDI;}else{M.push({id:nextId++,n,cat,p:pr,e:'🍽️',img:tDI,d:desc||n});}mark();clDM();rMG();rCN();rMD();toast('✅ 已保存');}
async function delD(id){const d=M.find(x=>x.id===id);if(!d||!confirm(`删除「${d.n}」？`))return;M=M.filter(x=>x.id!==id);cart=cart.filter(c=>c.id!==id);sc();mark();rMG();rCN();rMD();rCB();rCP();toast('🗑️ 已删除');}
...
async function delC(n){if(!confirm(`删除分类「${n}」？`))return;const fb=C.find(x=>x.name!==n)?.name||'';M.forEach(d=>{if(d.c===n)d.c=fb;});C=C.filter(c=>c.name!==n);mark();rMC();rMD();rMG();rCN();toast('🗑️ 已删除');}
...
async function delM(n){if(!confirm(`删除「${n}」？`))return;P=P.filter(m=>m.name!==n);cart=cart.filter(c=>c.m!==n);sc();mark();rMM();rMS();rCB();rCP();toast('🗑️ 已删除');}
