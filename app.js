// ============================================================
//  家庭点菜 — CloudBase 匿名登录版
//  无需注册账号 · 只需家庭密码 · 全自动云同步
// ============================================================
...
async function hL(){
  ...
  if(env&&!cfg){localStorage.setItem('fm_tcb',JSON.stringify({env}));await iCB();document.getElementById('cfgBar').style.display='none';}
  if(!gCfg()&&!env){document.getElementById('syncErr').textContent='请先填写CloudBase环境ID';document.getElementById('syncErr').style.display='block';return;}

  const pin=gPin();
  if(!pin){
    const p1=document.getElementById('pin1').value.trim(),p2=document.getElementById('pin2').value.trim();
    if(!p1)return;if(p1!==p2){document.getElementById('pinErr').textContent='两次输入不一致';document.getElementById('pinErr').classList.add('show');return;}
    localStorage.setItem('fm_pin',p1);fi=await ensureFam();
  }else{
    const p3=document.getElementById('pin3').value.trim();
    if(p3!==pin){document.getElementById('pinErr').textContent='密码错误';document.getElementById('pinErr').classList.add('show');return;}
    fi=gFid();
  }
  if(!fi){fi=await ensureFam();}
...
async function ensureFam(){
  ...
  if(fid)return fid;
  // 创建默认家庭
  const code=gC();const n='我的家庭';
  try{
    const r=await db.collection('families').add({name:n,code,createdAt:new Date()});
    fid=r.id;localStorage.setItem('fm_fid',fid);
    for(const d of DD)await db.collection('dishes').add({...d,fid});
    for(const c of DC)await db.collection('categories').add({...c,fid});
    for(const m of DM)await db.collection('members').add({...m,fid});
    return fid;
  }catch(e){return null;}
}
...
async function rstAll(){if(!confirm('⚠️ 清空所有数据并重置？'))return;localStorage.clear();const dbs=await indexedDB.databases();for(const d of dbs)indexedDB.deleteDatabase(d.name);location.reload();}

// === 数据 ===
async function rf(){sl('⏳ 同步中...');try{const[ds,cs,ms,os]=await Promise.all([db.collection('dishes').where({fid:fi}).limit(1000).get(),db.collection('categories').where({fid:fi}).limit(1000).get(),db.collection('members').where({fid:fi}).limit(1000).get(),db.collection('orders').where({fid:fi}).orderBy('createdAt','desc').limit(500).get()]);M=ds.data.map(r=>({id:r._id,...r}));C=cs.data.map(r=>({id:r._id,...r}));P=ms.data.map(r=>({id:r._id,...r}));O=os.data.map(r=>({id:r._id,...r}));sl('✅ 已同步');}catch(e){sl('⚠️ 同步失败');}rA();}
...
async function cO(){const n=document.getElementById('cnn').value.trim();try{await db.collection('orders').add({items:[...cart],total:tP(),note:n,fid:fi,createdAt:new Date()});cart=[];document.getElementById('cnn').value='';sc();lC();lM();await rf();rCB();rCP();toast('🎉 下单成功');}catch(e){toast('⚠️ 下单失败');}}
...
async function sC(){const on=document.getElementById('coid').value.trim(),nn=document.getElementById('cnm').value.trim(),em=document.getElementById('cem').value.trim();if(!nn){toast('⚠️ 请输入名称');return;}try{if(on){const c=C.find(x=>x.name===on);if(!c)return;if(on!==nn&&C.some(x=>x.name===nn)){toast('⚠️ 已存在');return;}await db.collection('categories').doc(c.id).update({name:nn,emoji:em||'📂'});for(const d of M.filter(x=>x.c===on))await db.collection('dishes').doc(d.id).update({c:nn});}else{if(C.some(x=>x.name===nn)){toast('⚠️ 已存在');return;}await db.collection('categories').add({name:nn,emoji:em||'📂',fid:fi});}clCM();await rf();toast('✅ 已保存');}catch(e){toast('⚠️ 保存失败');}}
