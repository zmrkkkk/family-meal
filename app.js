// === 家庭点菜 — 无登录 · 打开即用 · 自动云同步 ===
const G='zmrkkkk',R='family-meal'; // GitHub 用户名和仓库名（固定）
let T=''; // GitHub Token（存在 localStorage）
let M=[],C=[],P=[],O=[],cart=[],cur='all',sr='',cm='d',nextId=1000,tDI='',tMA='',syncOk=false;

// === Token ===
function getToken(){T=localStorage.getItem('fm_tk')||'';return T;}
function svTk(){const v=document.getElementById('tkInput').value.trim();if(!v){toast('请输入Token');return;}localStorage.setItem('fm_tk',v);T=v;document.getElementById('tkModal').classList.remove('open');init();}

// === 云端读写 ===
async function load(k){const url=`https://${G}.github.io/${R}/data/${k}.json?t=${Date.now()}`;try{const r=await fetch(url);if(!r.ok)return null;return await r.json();}catch(e){return null;}}
async function saveOne(k,data){
  if(!T)return false;
  const path=`/repos/${G}/${R}/contents/data/${k}.json`;
  try{
    let sha='';try{const cur=await ghGet(path);sha=cur.sha;}catch(e){}
    const json=JSON.stringify(data),bytes=new TextEncoder().encode(json);
    const bin=Array.from(bytes).map(b=>String.fromCharCode(b)).join('');
    const content=btoa(bin),body={message:'update '+k,content};if(sha)body.sha=sha;
    await ghGet(path,{method:'PUT',body:JSON.stringify(body)});return true;
  }catch(e){console.error('save error:',e);return false;}
}
async function ghGet(path,opts={}){
  const r=await fetch('https://api.github.com'+path,{...opts,headers:{...opts.headers,Authorization:'Bearer '+T,Accept:'application/vnd.github+json'}});
  if(!r.ok)throw new Error(r.status);return r.json();
}
async function saveAll(){if(!T)return;ssT('⏳');const ok=await Promise.all([saveOne('menu',M),saveOne('cats',C),saveOne('mems',P),saveOne('orders',O)]);ssT(ok.every(x=>x)?'✅ 已同步':'⚠️');}
async function syncNow(){init();toast('🔄 已刷新');}

// === 初始化 ===
async function init(){
  if(!getToken()){document.getElementById('tkModal').classList.add('open');return;}
  ssT('⏳ 同步中...');
  const[dm,dc,dp,do_]=await Promise.all([load('menu'),load('cats'),load('mems'),load('orders')]);
  M=dm||[];C=dc||[];P=dp||[];O=do_||[];
  let mx=0;M.forEach(d=>{if(d.id>mx)mx=d.id;});nextId=mx>=999?mx+1:1000;
  lc();cart=cart.filter(c=>M.some(d=>d.id===c.id));sc();
  rA();ssT(M.length>0?'✅ 已同步('+M.length+'菜)':'📡 云端无数据，请先添加菜品');syncOk=true;
}

// === 状态显示 ===
function ssT(m){document.getElementById('ss').textContent=m;}
...
// === 购物车 ===
function aT(id){const d=M.find(x=>x.id===id);if(!d)return;const m=document.getElementById('ms').value;if(!m){toast('⚠️ 请选择点菜人');return;}const ex=cart.find(c=>c.id===id&&c.m===m);if(ex)ex.q++;else cart.push({id,name:d.n,price:d.p,emoji:d.e,img:d.img||'',q:1,m});sc();rMG();rCB();rCP();}
...
async function clH(){if(!O.length)return;if(!confirm('清空全部订单？此操作不可恢复。'))return;O=[];await saveOne('orders',O);rH();toast('🗑️ 已清空');}
...
// === CRUD ===
function oD(id){tDI='';document.getElementById('dcat').innerHTML=C.map(c=>`<option value="${esc(c.name)}">${c.emoji||'📂'} ${esc(c.name)}</option>`).join('');if(id){const d=M.find(x=>x.id===id);if(!d)return;document.getElementById('dt').textContent='✏️ 编辑菜品';document.getElementById('di').value=d.id;document.getElementById('dn').value=d.n;document.getElementById('dcat').value=d.c;document.getElementById('dpr').value=d.p;document.getElementById('dde').value=d.d;if(d.img){tDI=d.img;document.getElementById('dp').innerHTML=`<img src="${d.img}">`;document.getElementById('dp').classList.add('has-image');document.getElementById('dcb').style.display='inline-block';}else clD();}else{document.getElementById('dt').textContent='🍽️ 添加菜品';document.getElementById('di').value='';['dn','dde'].forEach(x=>document.getElementById(x).value='');document.getElementById('dpr').value='';document.getElementById('dcat').value=C[0]?.name||'';clD();}document.getElementById('dm').classList.add('open');}
...
async function sD(){const id=document.getElementById('di').value,n=document.getElementById('dn').value.trim(),cat=document.getElementById('dcat').value,pr=parseInt(document.getElementById('dpr').value),desc=document.getElementById('dde').value.trim();if(!n){toast('⚠️ 请输入菜名');return;}if(!cat){toast('⚠️ 请选择分类');return;}if(isNaN(pr)||pr<0){toast('⚠️ 请输入有效价格');return;}if(id){const d=M.find(x=>x.id===parseInt(id));d.n=n;d.c=cat;d.p=pr;d.d=desc||n;d.img=tDI;}else{M.push({id:nextId++,n,cat,p:pr,e:'🍽️',img:tDI,d:desc||n});}await saveOne('menu',M);clDM();rMG();rCN();rMD();toast('✅ 已保存');}
async function delD(id){const d=M.find(x=>x.id===id);if(!d||!confirm(`删除「${d.n}」？`))return;M=M.filter(x=>x.id!==id);cart=cart.filter(c=>c.id!==id);sc();await saveOne('menu',M);rMG();rCN();rMD();rCB();rCP();toast('🗑️ 已删除');}
...
async function sC(){const on=document.getElementById('coid').value.trim(),nn=document.getElementById('cnm').value.trim(),em=document.getElementById('cem').value.trim();if(!nn){toast('⚠️ 请输入名称');return;}if(on){if(on!==nn&&C.some(x=>x.name===nn)){toast('⚠️ 名称已存在');return;}const c=C.find(x=>x.name===on);c.name=nn;c.emoji=em||'📂';M.forEach(d=>{if(d.c===on)d.c=nn;});await saveOne('menu',M);}else{if(C.some(x=>x.name===nn)){toast('⚠️ 名称已存在');return;}C.push({name:nn,emoji:em||'📂'});}await saveOne('cats',C);clCM();rMC();rMD();rMG();rCN();toast('✅ 已保存');}
async function delC(n){if(!confirm(`删除分类「${n}」？该分类下的菜品将移到第一个分类。`))return;const fb=C.find(x=>x.name!==n)?.name||'';M.forEach(d=>{if(d.c===n)d.c=fb;});C=C.filter(c=>c.name!==n);await saveOne('menu',M);await saveOne('cats',C);rMC();rMD();rMG();rCN();toast('🗑️ 已删除');}
...
async function sM(){const on=document.getElementById('moid').value.trim(),nn=document.getElementById('mnm').value.trim();if(!nn){toast('⚠️ 请输入称呼');return;}if(on){const i=P.findIndex(x=>x.name===on);if(on!==nn&&P.some(x=>x.name===nn)){toast('⚠️ 称呼已存在');return;}if(i>=0){P[i].name=nn;P[i].a=tMA;}cart.forEach(c=>{if(c.m===on)c.m=nn;});sc();}else{if(P.some(x=>x.name===nn)){toast('⚠️ 称呼已存在');return;}P.push({name:nn,a:tMA});}await saveOne('mems',P);clMM();rMM();rMS();rCP();toast('✅ 已保存');}
async function delM(n){if(!confirm(`删除「${n}」？`))return;P=P.filter(m=>m.name!==n);cart=cart.filter(c=>c.m!==n);sc();await saveOne('mems',P);rMM();rMS();rCB();rCP();toast('🗑️ 已删除');}
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  init();
  // 每30秒自动同步
  setInterval(()=>{if(document.hidden)return;init();},30000);
});
